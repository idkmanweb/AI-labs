...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split in possible_splits:
            ...
            if left != 0 and right != 0:
                gini_left = 1 - pow(left_pos / (left_neg + left_pos), 2) - pow(left_neg / (left_neg + left_pos), 2)
                gini_right = 1 - pow(right_pos / (right_neg + right_pos), 2) - pow(right_neg / (right_neg + right_pos),
                                                                                   2)
                gini_gain = 1 - left / (right + left) * gini_left - right / (right + left) * gini_right
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        for d in range(X.shape[1]) if feature_subset is None else np.random.choice(X.shape[1], feature_subset, replace=False):
